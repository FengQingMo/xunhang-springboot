...
public class PrivateMessageServiceImpl extends ServiceImpl<PrivateMessageMapper, PrivateMessage> implements IPrivateMessageService {
    ...
    public Long sendMessage(PrivateMessageDTO dto) {
        ...
        Boolean isFriends = friendService.isFriend(UserHolder.getCurrentId(), dto.getRecvId());
        ...
        msg.setSendId(UserHolder.getCurrentId());
        ...
        sendMessage.setSender(new IMUserInfo(UserHolder.getCurrentId(), 1));
        sendMessage.setRecvId(msgInfo.getRecvId());
        sendMessage.setSendToSelf(true);
        sendMessage.setData(msgInfo);
        sendMessage.setSendResult(true);
        imClient.sendPrivateMessage(sendMessage);
        log.info("发送私聊消息，发送id:{},接收id:{}，内容:{}", UserHolder.getCurrentId(), dto.getRecvId(), dto.getContent());
        return msg.getId();
    }
    ...
    public void recallMessage(Long id) {
        ...
        if (!msg.getSendId().equals(UserHolder.getCurrentId())) {
    ...
    public List<PrivateMessageVO> loadMessage(Long minId) {
        List<Friend> friends = friendService.findFriendByUserId(UserHolder.getCurrentId());
        if (friends.isEmpty()) {
        ...
        queryWrapper.gt(PrivateMessage::getId, minId)
                ...
                .ne(PrivateMessage::getStatus, MessageStatus.RECALL.code())
                .and(wrap -> wrap.and(
                        wp -> wp.eq(PrivateMessage::getSendId, UserHolder.getCurrentId())
                                .in(PrivateMessage::getRecvId, friendIds))
                        .or(wp -> wp.eq(PrivateMessage::getRecvId, UserHolder.getCurrentId())
        ...
        List<Long> ids = messages.stream()
                .filter(m -> !m.getSendId().equals(UserHolder.getCurrentId()) && m.getStatus().equals(MessageStatus.UNSEND.code()))
        ...
        log.info("拉取消息，用户id:{},数量:{}", UserHolder.getCurrentId(), messages.size());
    ...
    public void pullOfflineMessage(Long minId) {
        if(!imClient.isOnline(UserHolder.getCurrentId())){
        ...
        List<PrivateMessage> messages = query().gt("id", minId)
                ...
                .and(wrap -> wrap.and(wp -> wp.eq("send_id", UserHolder.getCurrentId())
                                .in("recv_id", friendIds))
                        .or(wp -> wp.eq("recv_id", UserHolder.getCurrentId())
        ...
        for(PrivateMessage m:messages ){
            ...
            sendMessage.setRecvId(UserHolder.getCurrentId());
            //todo
        ...
        // 关闭加载中标志
        this.sendLoadingMessage(false);
        log.info("拉取私聊消息，用户id:{},数量:{}", UserHolder.getCurrentId(), messages.size());
    }
    ...
    public void readedMessage(Long friendId) {
        ...
        msgInfo.setSendId(UserHolder.getCurrentId());
        ...
        updateWrapper.eq(PrivateMessage::getSendId, friendId)
                .eq(PrivateMessage::getRecvId, UserHolder.getCurrentId())
                .eq(PrivateMessage::getStatus, MessageStatus.SENDED.code())
                .set(PrivateMessage::getStatus, MessageStatus.READED.code());
        this.update(updateWrapper);
        log.info("消息已读，接收方id:{},发送方id:{}", UserHolder.getCurrentId(), friendId);
    }
    ...
    public Long getMaxReadedId(Long friendId) {
        ...
        wrapper.eq(PrivateMessage::getSendId, UserHolder.getCurrentId())
                .eq(PrivateMessage::getRecvId, friendId)
